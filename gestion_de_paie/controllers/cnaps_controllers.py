...
class ExportReportCnapsController(http.Controller):
    ...
    def report_excel_employer(self, workbook, p, m, c, y, bold, full_border, data_month1, data_month2, data_month3,
        ...
        worksheet_emp.write('C4', comp_inf_['matricule'], left)
        ...
        worksheet_emp.write('C13', None, full_border)
        worksheet_emp.write('B15', "Taux Employeur", left)
        worksheet_emp.write('B16', "Taux Travailleur", left)
        worksheet_emp.write('C15', str(plf['patr']) + '%', full_border)
        worksheet_emp.write('C16', str(plf['emp']) + '%', full_border)
        worksheet_emp.write('A18', u"Mois concerné", bold)
        worksheet_emp.write('A20', "RECAPITULATION", bold)
        worksheet_emp.write('C18', month['mth1'], full_border)
        worksheet_emp.write('D18', month['mth2'], full_border)
        worksheet_emp.write('E18', month['mth3'], full_border)
        worksheet_emp.write('B22', "Effectifmensuel(OBLIGATOIRE)", full_border)
        worksheet_emp.write('C22', int(count_eff['count_mth1']), full_border)
        worksheet_emp.write('D22', int(count_eff['count_mth2']), full_border)
        worksheet_emp.write('E22', int(count_eff['count_mth3']), full_border)
        worksheet_emp.write('B23', u"Totaux Salaires plafonnés", full_border)
        worksheet_emp.write('B24', "Cotisations Employeur", full_border)
        n1 = literal_eval(data_month1)
        n2 = literal_eval(data_month2)
        n3 = literal_eval(data_month3)
        worksheet_emp.write_formula('C23', "='Mois 1'!L{}".format(len(n1) + 3), m_fmt)
        worksheet_emp.write_formula('D23', "='Mois 2'!L{}".format(len(n2) + 3), m_fmt)
        worksheet_emp.write_formula('E23', "='Mois 3'!L{}".format(len(n3) + 3), m_fmt)
        worksheet_emp.write_formula('C24', "='Mois 1'!M{}".format(len(n1) + 3), m_fmt)
        worksheet_emp.write_formula('D24', "='Mois 2'!M{}".format(len(n2) + 3), m_fmt)
        worksheet_emp.write_formula('E24', "='Mois 3'!M{}".format(len(n3) + 3), m_fmt)
        worksheet_emp.write('B25', "Cotisations travailleurs", full_border)
        worksheet_emp.write_formula('C25', "='Mois 1'!N{}".format(len(n1) + 3), m_fmt)
        worksheet_emp.write_formula('D25', "='Mois 2'!N{}".format(len(n2) + 3), m_fmt)
        worksheet_emp.write_formula('E25', "='Mois 3'!N{}".format(len(n3) + 3), m_fmt)
        worksheet_emp.write('F21', "TOTAUX", self.bold(workbook, 'left', 10, 0, True))
        worksheet_emp.write_formula('F22', "=sum(C22:E22)", full_border)
    ...
    def month(self, workbook, data_month, n, plf):
        ...
        self.empty_col(worksheet_mth, 2, 9, cl, align_center)
